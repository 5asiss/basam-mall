...
# [수정됨] 서버 환경에 맞춘 이미지 저장 경로 설정
# (C드라이브 경로 삭제 -> 현재 폴더 내 'images' 폴더로 변경)
...
def upload_excel():
    try:
        ...
        for idx, row in df.iterrows():
            ...
            icon_data = '🍎'
            if img_filename:
                # [주의] 웹 서버에서는 사용자 PC(C드라이브)에 접근 불가
                # 엑셀 업로드 시 이미지는 별도로 관리자가 사진을 찍어 올리거나
                # 서버의 'images' 폴더에 미리 파일이 있어야 함.
